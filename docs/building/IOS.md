...
This guide walks you through setting up and building the zmNg iOS app from scratch.
...
To enable push notifications, you need to configure Firebase Cloud Messaging (FCM) and Apple Push Notification Service (APNs):
...
You need to create an APNs authentication key in your Apple Developer account:
...
4. Enter your **Key ID** and **Team ID** (found in Apple Developer portal)
5. Click **Upload**
...
Once Xcode opens:
1. Select your target device or simulator from the dropdown
...
Logs are best viewed in Xcode's console when running via `npm run ios:open`.
...
5. Ensure **Automatically manage signing** is checked (or configure manual signing)
...
5. Once uploaded, you can distribute via TestFlight or submit for App Store review

**For Ad Hoc Distribution:**
1. Click **Distribute App**
2. Select **Ad Hoc**
3. Follow the prompts to create an IPA file
4. Distribute the IPA to testers
...
- For automatic signing, ensure "Automatically manage signing" is enabled
- For manual signing, ensure you have the correct provisioning profiles installed
...
- **Distribution**: Upload to App Store Connect for TestFlight or App Store release
